...
const App = () => {
  ...
  const socketUrl = 'WS://play1.canadacentral.cloudapp.azure.com:8090/transaction-data/BTCCAD';
  ...
  const [valuebtc, setValuebtc] = useState({ last: '', priceChange: '', percentageChange: '' });
  const handleKeypress = async (event) => {
    if (event.key === 'Enter') {
      const data = await fetch(`http://play1.canadacentral.cloudapp.azure.com:8090/api/transaction/getLastQuote?symbol=/${event.target.value}`);
      const res = await data.json()
      setValuebtc(res.data);
    }
  }
